...
export const TweetsApi = {
  ...
  async fetchTweets(userId?: string): Promise<Tweet[]> {
    const {data} = await axios.get<Response<Tweet[]>>(userId ? `https://twitter-clone-mongodb-server.herokuapp.com/tweets/user/${userId}` : 'https://twitter-clone-mongodb-server.herokuapp.com/tweets');
    return data.data;
  },
  //запрос на сервер по скачиванию с него твита с идентификатором id
  async fetchTweetData(id: string): Promise<Tweet> {
    const {data} = await axios.get<Response<Tweet>>('https://twitter-clone-mongodb-server.herokuapp.com/tweets/'+id);
    return data.data;
  },
  ...
  async addTweet(payload: {text: string; images: string[]}): Promise<Tweet> {
    const {data} = await axios.post<Response<Tweet>>('https://twitter-clone-mongodb-server.herokuapp.com/tweets', payload);
    return data.data;
  },
  //запрос на удаление с сервера твита по его идентификатору
  removeTweet: (id: string): Promise<void> => axios.delete('https://twitter-clone-mongodb-server.herokuapp.com/tweets/' + id),
};