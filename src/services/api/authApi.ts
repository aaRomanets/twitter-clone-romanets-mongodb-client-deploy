...
export const AuthApi = {
    ...
    async signIn(postData: LoginFormProps): Promise<ResponseApi> {
        const { data } = await axios.post<ResponseApi>('https://twitter-clone-mongodb-server.herokuapp.com/auth/login', { username: postData.email, password: postData.password });
        return data;
    },
    //запрос на сервер по авторизации пользователя
    async signUp(postData: RegisterFormProps): Promise<ResponseApi> {
        const { data } = await axios.post<ResponseApi>('https://twitter-clone-mongodb-server.herokuapp.com/auth/register', { email: postData.email, username: postData.username, fullname: postData.fullname, password: postData.password, password2: postData.password2 });
        return data;
    },
    ...
    async getMe(): Promise<ResponseApi> {
        const { data } = await axios.get<ResponseApi>('https://twitter-clone-mongodb-server.herokuapp.com/users/me');
        return data;
    },
    ...
    async getUserInfo(userId: string): Promise<ResponseApi> {
        const { data } = await axios.get<ResponseApi>('https://twitter-clone-mongodb-server.herokuapp.com/users/' + userId);
        return data;
    },
};
